...
每个规则都有一个名字, 使用字符串类型的`name`属性指定. 该名称必须是一个语法有效的目标名称，如上所述. 在某些情况下，名称有些随意，更有趣的是规则生成的文件的名称; 这是真实的genrules。 在其他情况下，名称是重要的：对于`* _binary`和

`* _test`规则，例如，规则名称决定了构建的可执行文件的名称。
